...
-- Source: supabase/migrations/202609120001_student_access.sql
begin;
...
-- Source: supabase/migrations/202609150001_nura_access.sql
begin;
...
-- Source: supabase/migrations/202609160002_member_access_summary.sql
begin;
...
begin
 perform private.require_staff();
 return query select p.id,exists(select 1 from private.staff st where st.user_id=p.id),exists(select 1 from private.students su where su.user_id=p.id),exists(select 1 from private.nura_members nm where nm.user_id=p.id),(select m.tier from public.memberships m where m.user_id=p.id and now()>=m.starts_at and now()<m.expires_at order by m.starts_at desc limit 1) from public.profiles p;
end; $$;
