...
function message(job:{kind:string,payload:Record<string,unknown>}){
 const p=job.payload, name=escape(p.name||'there');
 const subject=String(p.subject||'Santuri booking update');
 if(job.kind==='role_assigned'){
  const role=escape(p.role_label||'access');
  const text=`Hello ${p.name||'there'},\n\nThe Santuri team has assigned ${p.role_label||'new access'} to your account. You can now sign in to view the available booking options.\n\nSign in: https://bookings.santuri.org/book.html\n\nSanturi East Africa\nbookings@santuri.org`;
  const html=`<div style="font-family:Arial,sans-serif;line-height:1.5;color:#111;max-width:600px"><p>Hello ${name},</p><p>The Santuri team has assigned <strong>${role}</strong> to your account.</p><p>You can now <a href="https://bookings.santuri.org/book.html">sign in to view the available booking options</a>.</p><p>Santuri East Africa<br><a href="mailto:bookings@santuri.org">bookings@santuri.org</a></p></div>`;
  return {from:`${senderName} <${sender}>`,to:String(p.recipient||''),subject:'Santuri access updated',text,html};
 }
 const space=escape(p.space), date=escape(dateLabel(String(p.starts_at))), start=escape(timeLabel(String(p.starts_at))), end=escape(timeLabel(String(p.ends_at))), note=String(p.note||'').trim();
